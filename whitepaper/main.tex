...
\item \textbf{Shift reduction (\S\ref{sec:shift-reduction}).} The evaluation claims produced by the Zero, BitAnd, IntMul, and BinMul reductions are batched, and a two-phase sumcheck reduces them to a single evaluation claim on $\mle{w}$ at a point $(r_j, r_y) \in K^6 \times K^{\ell_\text{words}}$.
...
\end{equation}
This nested sum is over $\ell_\text{words} + 21$ variables; running sumcheck on it naively is prohibitive. The next subsubsection shows how to factor it into two manageable phases.

\subsubsection{The Two-Phase Sumcheck}
\label{sec:two-phase-sumcheck}

\paragraph{Factorization.}
In \eqref{eq:shift-reduction-claim}, no single inner factor depends on both $i$ and $y$: $\delta_D(r_{\widehat{i}}, \widehat{i})$ and $\mle{\textsf{shift-ind}}(i, j, s, o)$ contain $i$ but not $y$, whereas $\mle{Z}_\text{cons}(r'_x, y, s, o)$ and $\mle{w}(j, y)$ contain $y$ but not $i$. We can therefore factor the two innermost sums and obtain
\[
  \alpha_z \stackrel{?}{=} \sum_{j \in \BB_6} \sum_{s \in \BB_6} \sum_{o \in \BB_3} h(j, s, o) \cdot g(j, s, o),
\]
where
\begin{align*}
...
\end{align*}
Both $h$ and $g$ are multilinear in the $15$ variables $(J, S, O)$: for $h$, the variable factors $\mle{\textsf{shift-ind}}(i, J, S, O)$ depend on $J, S, O$ multilinearly and $\delta_D(r_{\widehat{i}}, \widehat{i})$ is constant in them; for $g$, $\mle{Z}_\text{cons}(r'_x, y, S, O)$ depends on $S, O$ but not $J$ while $\mle{w}(J, y)$ depends on $J$ but not $S, O$, so each summand is multilinear in $(J, S, O)$.

\paragraph{Phase 1: $15$ rounds.}
The parties run a $15$-round sumcheck on
\[
  \alpha_z \stackrel{?}{=} \sum_{(j, s, o) \in \BB_{15}} h(j, s, o) \cdot g(j, s, o).
\]
The composing polynomial is the degree-$2$ product of two multilinears; the prover sends a degree-$2$ round polynomial in each round. The variables are bound in the order $S_0, \ldots, S_5, O_0, \ldots, O_2, J_0, \ldots, J_5$, so that the nine variables carrying the shift amount and the shift operation are bound before the six carrying the bit index; the prover algorithm of \S\ref{sec:shift-reduction-prover} exploits this order. After $15$ rounds, the sumcheck reduces $\alpha_z$ to a claim
\[
  \beta \stackrel{?}{=} h(r_j, r_s, r_o) \cdot g(r_j, r_s, r_o),
\]
where $r_s \in K^6$, $r_o \in K^3$, and $r_j \in K^6$ are the sumcheck challenges, in the order drawn. The verifier can evaluate $h(r_j, r_s, r_o)$ on its own: it expands the interpolated indicator by \eqref{eq:shift-indicator-interpolation} into the eight succinct shift-indicator arithmetizations of \S\ref{sec:shift-indicators}, weighted by $\textsf{TensorExpand}_3(r_o)$, after a single $O(2^6)$ precomputation of $\bigl( \delta_D(r_{\widehat{i}}, \widehat{i}) \bigr)_{i \in \BB_6}$.

\paragraph{Phase 2: $\ell_\text{words}$ rounds.}
Unrolling the definition of $g(r_j, r_s, r_o)$ and exchanging sums,
\[
  \beta \stackrel{?}{=} \sum_{y \in \BB_{\ell_\text{words}}} \Bigl[ h(r_j, r_s, r_o) \cdot \mle{Z}_\text{cons}(r'_x, y, r_s, r_o) \Bigr] \cdot \mle{w}(r_j, y).
\]
...
\[
  \gamma \stackrel{?}{=} \Bigl[ h(r_j, r_s, r_o) \cdot \mle{Z}_\text{cons}(r'_x, r_y, r_s, r_o) \Bigr] \cdot \mle{w}(r_j, r_y).
\]
...
\paragraph{Verifier evaluation of the constraint matrix.}
At the very end, the verifier needs the single evaluation $\mle{Z}_\text{cons}(r'_x, r_y, r_s, r_o)$. The straightforward sparse-evaluation algorithm runs in $O(n_\text{cons}) + O(\text{circuit size})$ time by iterating over the constraint lists; succinct verification is left for future work, following e.g.\ the Spark compiler of Setty~\cite{Setty:2020aa}.

\subsubsection{Prover Algorithm}
\label{sec:shift-reduction-prover}

We sketch the prover-side data structures and the main loops; the goal is to make the prover's work close to linear in the witness size despite the formal $\ell_\text{words} + 21$-dimensional sum.
