...
\]
The composing polynomial is the degree-$2$ product of two multilinears; the prover sends a degree-$2$ round polynomial in each round. After $15$ rounds, the sumcheck reduces $\alpha_z$ to a claim
\[
  \beta \stackrel{?}{=} h(r_j, r_s, r_o) \cdot g(r_j, r_s, r_o),
\]
where $r_j, r_s \in K^6$ and $r_o \in K^3$ are the sumcheck challenges. The verifier can evaluate $h(r_j, r_s, r_o)$ on its own: it expands the interpolated indicator by \eqref{eq:shift-indicator-interpolation} into the eight succinct shift-indicator arithmetizations of \S\ref{sec:shift-indicators}, weighted by $\textsf{TensorExpand}_3(r_o)$, after a single $O(2^6)$ precomputation of $\bigl( \delta_D(r_{\widehat{i}}, \widehat{i}) \bigr)_{i \in \BB_6}$.
...
Using $\textsf{index}$, the $2^{15}$-entry table $g[j][s][o] = \sum_y \mle{Z}_\text{cons}(r'_x, y, s, o) \cdot \mle{w}(j, y)$ is computed by iterating over $y$ and, for each populated key $(s, o)$ in $\textsf{index}[y]$, adding $\textsf{index}[y][(s, o)]$ into $g[j][s][o]$ for each bit $j$ where $w[y]_j = 1$. Total work is proportional to the (sparse) circuit size.
