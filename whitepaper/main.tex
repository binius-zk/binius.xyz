...
\]
since the bivariate $\mle{\eq}(0, X) = 1$ for the current variable and the accumulated constant $\mle{\eq}_i(0^i, \cdot) = 1$ are both identically $1$. The prover touches $F$ not at all.
\item In a round $i \ge \nu$, all padding variables are bound and contribute the constant $c \coloneqq \mle{\eq}_\nu\bigl( 0^\nu, (r_n, \ldots, r_{n + \nu - 1}) \bigr) = 1$, so the round polynomial coincides with the unpadded one,
\[
  R^{(i)}(X) = c \cdot R_F^{(i - \nu)}(X) = R_F^{(i - \nu)}(X),
\]
...
\begin{quote}
\textbf{Note (Boolean-basis convention in this subsection).} The analysis below is stated over the Boolean hypercube $\set{0, 1}^n$ and predates the monomial-basis (infinity-hypercube) representation adopted in \S\ref{sec:notation}. Its central mechanism---the powers of two $2^{n - i - 1}$ that arise from summing constants over $\set{0, 1}^n$ and collapse to $0$ in characteristic $2$---does not appear under summation over the infinity hypercube $\set{0, \infty}^n$, where summing a constant over a coordinate contributes only its value at $0$ plus a vanishing leading coefficient. The masking analysis below, and the repair via the multilinear-extension check, therefore require re-derivation in the monomial basis---under which the characteristic-$2$ failure may no longer occur. Pending that re-derivation, we retain the original Boolean-basis treatment here.
\end{quote}
