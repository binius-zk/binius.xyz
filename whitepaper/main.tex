...
\]
We will define BitAnd, IntMul, and BinMul constraints in terms of arrays of this form.
...
The LIOP proceeds in eight phases:
\begin{enumerate}
...
\item \textbf{Shift reduction (\S\ref{sec:shift-reduction}).} The evaluation claims produced by the BitAnd, IntMul, and BinMul reductions are batched, and a two-phase sumcheck reduces them to a single evaluation claim on $\mle{w}$ at a point $(r_j, r_y) \in K^6 \times K^{\ell_\text{words}}$.
...
The two multiplication reductions (phases 2 and 3) must precede the BitAnd reduction (phase 4): the BitAnd univariate skip samples the shared bit-index oblong point $r_{\widehat{i}}$, and soundness requires that the multiplication reductions' per-bit outputs already be fixed when $r_{\widehat{i}}$ is drawn (\S\ref{sec:rz-unification}). Phases 2 and 3 are mutually independent and may run in either order.
