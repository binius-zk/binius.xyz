...
\]
using the leaf constant $g^{2^i}$ for $a$ and $c_\text{lo}$ and the high-half constant $g^{2^{64 + i}}$ for $c_\text{hi}$; for $b$ it tests $\sum_{i \in \BB_6} \mle{\eq}_6(r_I^b, i) \, \mle{b}(i, r_x)$ against $\mle{b}(r_I^b, r_x)$, and it tests the $i = 0$ values against the wraparound claims. The sent values are thereby bound to the GKR-verified products, and to the committed witness by the shift reduction downstream.
