...
The two multiplication reductions (phases 2 and 3) must precede the BitAnd reduction (phase 4): the BitAnd univariate skip samples the shared bit-index oblong point $r_{\widehat{i}}$, and soundness requires that the multiplication reductions' per-bit outputs already be fixed when $r_{\widehat{i}}$ is drawn (\S\ref{sec:rz-unification}). Phases 2 and 3 are mutually independent and may run in either order. Phase 5 reuses the BitAnd challenges and so is placed after phase 4; since it carries no message, this is a matter of bookkeeping rather than of interaction.
...
Both parties reuse the challenges of the BitAnd reduction (\S\ref{sec:univariate-skip}): the deterministic-zerocheck challenge $r_x = (\sigma_0, \sigma_1, \sigma_2, \overline{r}_{x, 0}, \ldots, \overline{r}_{x, \ell_\text{and} - 4})$ of step 1, and the univariate-skip challenge $r_{\widehat{i}} \in K$ of step 3. Alongside $\overline{r}_x$ the verifier additionally samples $\overline{r}_d \gets K^{\max(0, \ell_\text{zero} - \ell_\text{and})}$, and both parties set $\rho \in K^{\ell_\text{zero}}$ to be the length-$\ell_\text{zero}$ prefix of
\[
  \bigl( \sigma_0, \sigma_1, \sigma_2, \overline{r}_{x, 0}, \ldots, \overline{r}_{x, \ell_\text{and} - 4}, \overline{r}_{d, 0}, \ldots \bigr),
\]
that is, a prefix of $r_x$ when $\ell_\text{zero} \leq \ell_\text{and}$ and $r_x$ extended by fresh challenges otherwise. Since $\ell_\text{zero} \geq 3$ (\S\ref{sec:binius-liop}), the point $\rho$ opens with the three deterministic coordinates $\sigma_0, \sigma_1, \sigma_2$, and its remaining $\ell_\text{zero} - 3$ coordinates are uniform on $K$ and mutually independent.
...
\paragraph{Soundness.}
Suppose the prover data violates some Zero constraint, so that $d[i^*, x^*] = 1$ for some $i^* \in \BB_6$ and $x^* \in \BB_{\ell_\text{zero}}$. Write $x^* = u^* \mathbin{\Vert} v^*$ with $u^* \in \BB_3$ and $v^* \in \BB_{\ell_\text{zero} - 3}$, let $\overline{\rho} \coloneqq (\rho_3, \ldots, \rho_{\ell_\text{zero} - 1})$ be the random part of $\rho$, and set
\[
  \overline{d}(\widehat{I}, V) \coloneqq \sum_{u \in \BB_3} \widehat{d}(\widehat{I}, u, V) \cdot \mle{\eq}_3(\sigma, u),
\]
an oblong-multilinear in $1 + (\ell_\text{zero} - 3)$ variables. Because $\widehat{d}$ is multilinear in $X_0, X_1, X_2$, specializing those variables at $\sigma$ is precisely multilinear interpolation, so
\[
  \overline{d}(\widehat{I}, \overline{\rho}) = \widehat{d}(\widehat{I}, \rho);
\]
the deterministic coordinates of $\rho$ have been absorbed into $\overline{d}$, and it suffices to bound $\Pr[\overline{d}(r_{\widehat{i}}, \overline{\rho}) = 0]$.

First, $\overline{d}$ is not the zero polynomial. Evaluating at the cube point $(\widehat{i^*}, v^*) \in D \times \BB_{\ell_\text{zero} - 3}$ and using $\widehat{d}(\widehat{i}, x) = d[i, x]$ for $\widehat{i} \in D$,
\[
  \overline{d}(\widehat{i^*}, v^*) = \sum_{u \in \BB_3} d[i^*, u \mathbin{\Vert} v^*] \cdot \mle{\eq}_3(\sigma, u),
\]
an $\FF{2}$-linear combination of $\bigl( \mle{\eq}_3(\sigma, u) \bigr)_{u \in \BB_3}$ whose coefficient at $u = u^*$ is $1$. That list is $\FF{2}$-linearly independent in $K$ by the choice of $\sigma$ (\S\ref{sec:rijndael}), so $\overline{d}(\widehat{i^*}, v^*) \neq 0$. This is the same argument that licenses the deterministic zerocheck of \S\ref{sec:rijndael}, and it applies here for the same reason: the constraint array is an array of \emph{bits}, so $\widehat{d}$ is $\FF{2}$-valued on $D \times \BB_{\ell_\text{zero}}$, and an $\FF{2}$-combination of an $\FF{2}$-independent list vanishes only if every coefficient does. Along these three constraint-index axes the deterministic challenges give away nothing; substituting $\sigma$ is a change of $\FF{2}$-basis, not a random probe.

Second, $\overline{d}$ is fixed before $r_{\widehat{i}}$ and $\overline{\rho}$ are drawn. The array $d$ is a function of the prover data $w$, which the packed-witness oracle of phase 1 pins down before any challenge is sampled, so a prover cannot adapt $d$ to the point at which it will be evaluated. As $\overline{d}$ has degree $< 64$ in $\widehat{I}$ and degree $\leq 1$ in each of the remaining $\ell_\text{zero} - 3$ variables, and $(r_{\widehat{i}}, \overline{\rho})$ is uniform on $K^{1 + (\ell_\text{zero} - 3)}$, Schwartz--Zippel gives
\[
  \Pr\bigl[ \widehat{d}(r_{\widehat{i}}, \rho) = 0 \bigr] = \Pr\bigl[ \overline{d}(r_{\widehat{i}}, \overline{\rho}) = 0 \bigr] \leq \frac{63 + (\ell_\text{zero} - 3)}{|K|}.
\]
...
Note where the randomness of $r_{\widehat{i}}$ is spent. The $\FF{2}$-independence argument was applied to three of the \emph{constraint-index} axes; the bit-index axis was left to Schwartz--Zippel, which is why $r_{\widehat{i}}$ must be unpredictable even though $\rho$ is only partly so.

\paragraph{On reusing the BitAnd challenges.}
Sharing $r_{\widehat{i}}$ and $r_x$ across the BitAnd and Zero reductions is sound despite the resulting correlation. The bound above is a statement about a bad event for the Zero constraints, and it appeals only to the marginal distribution of $(r_{\widehat{i}}, \overline{\rho})$, which reuse leaves untouched; the analysis of \S\ref{sec:univariate-skip} does the same for its own bad event, and neither conditions on the other. Unsatisfiable prover data violates a constraint of some class, and the reduction for that class catches it with the probability computed for it in isolation. What both analyses do require is that every challenge be drawn after $w$ is committed, which the phase ordering of \S\ref{sec:protocol-overview} guarantees.
...
Only the oblong point must be shared. The reductions close at different constraint points---$r'_x$ for BitAnd, $\rho$ for Zero, and each multiplication reduction's own final challenge---and a constraint point enters the derivation below solely through the constraint matrices $\mle{Z}_{\text{cons}, \text{op}}$ of \S\ref{sec:mathematizing}, which are per-array objects to begin with. Batching arrays $z$ at points $\rho_z$ with coefficients $\eta_z$ therefore just replaces the single term $\mle{Z}_{\text{cons}, \text{op}}(r'_x, y, S)$ by $\sum_z \eta_z \cdot \mle{Z}^z_{\text{cons}, \text{op}}(\rho_z, y, S)$ throughout: the factorization of \S\ref{sec:two-phase-sumcheck} is untouched, the prover accumulates the sparse index of \S\ref{sec:shift-reduction-prover} from one tensor expansion per distinct point, and the verifier evaluates the per-array contributions separately at the end. The oblong point, by contrast, is shared out of necessity---it is carried by the factor $h_\text{op}$, which the phase-1 sumcheck binds jointly for all arrays.
