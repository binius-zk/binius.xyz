...
Identity \eqref{eq:exp-identity} is verified by composing two generic exponentiation subprotocols. Throughout, fix the generator $g \in K^*$ and let $z \in \FF{2}^{\BB_k \times \BB_{\ell_\text{mul}}}$ be a bit vector, with $k = 6$ and entries $z[i, x] \in \FF{2}$. We index the $2^k = 64$ bit positions by $i \in \set{0, \ldots, 2^k - 1}$, identified with the hypercube $\BB_k$ via binary representation. Its multilinear extension $\mle{z}(I, X) \in K[I_0, \ldots, I_{k - 1}, X_0, \ldots, X_{\ell_\text{mul} - 1}]^\mlin$ has $\mle{z}(i, X)$ for the multilinear section at the $i$-th hypercube point, with $\mle{\eq}_k(\cdot, i)$ the corresponding Lagrange basis function. The integer encoded by each column is $\intval{z}[x] = \sum_{i = 0}^{2^k - 1} 2^i \cdot z[i, x]$, with $\intval{\cdot}$ as in \S\ref{sec:intmul-constraints}.
...
\]
We denote by $\mle{\textsf{gen-exp}}(z)$ and $\mle{\textsf{var-exp}}(f, z)$ their multilinear extensions. The fixed-base subprotocol (\S\ref{sec:fixed-base-exp}) reduces an evaluation claim on $\mle{\textsf{gen-exp}}(z)$ to the $2^k$ per-bit evaluation claims $\mle{z}(i, r'_x)$; the variable-base subprotocol (\S\ref{sec:var-base-exp}) reduces an evaluation claim on $\mle{\textsf{var-exp}}(f, z)$ to one evaluation claim on $\mle{f}$ together with the $2^k$ per-bit claims $\mle{z}(i, r'_x)$. Both run the same GKR product tree that splits the exponent into its $2^k$ bits, differing only in how the product-tree leaves are discharged into the per-bit claims.

\paragraph{The product tree.}
Define the \emph{selector} $\sel(s, v) \coloneqq 1 + (v - 1) s$, which returns $v$ when the bit $s = 1$ and $1$ when $s = 0$. Collect the $2^k$ leaves into a vector $L \in K^{\BB_k \times \BB_{\ell_\text{mul}}}$ with entries
\[
  L[i, x] \coloneqq \bigl( f[x]^{2^i} \bigr)^{z[i, x]} = \sel\bigl( z[i, x],\, f[x]^{2^i} \bigr), \qquad i \in \BB_k, \quad x \in \BB_{\ell_\text{mul}},
\]
where the second equality uses that $z[i, x] \in \set{0, 1}$. The operator output is the product of the leaves over the bit index,
\[
  \textsf{var-exp}(f, z)[x] = \prod_{i \in \BB_k} L[i, x]
\]
(for the fixed-base operator, $f[x] \equiv g$ and the product is $\textsf{gen-exp}(z)[x]$). We evaluate this product by a balanced binary tree of depth $k$, whose layers are vectors $L^{(j)} \in K^{\BB_{k - j} \times \BB_{\ell_\text{mul}}}$, for $j = 0, \ldots, k$, with leaf layer $L^{(0)} \coloneqq L$ and each node the product of its two children,
\[
  L^{(j)}[i, x] = L^{(j - 1)}[0, i, x] \cdot L^{(j - 1)}[1, i, x], \qquad i \in \BB_{k - j}, \quad x \in \BB_{\ell_\text{mul}},
\]
the leading bit selecting the child; the root is $L^{(k)} = \textsf{var-exp}(f, z)$. Write $\mle{L}^{(j)}$ for the multilinear extension of the layer vector $L^{(j)}$.

Starting from the root claim $s^{(k)} \stackrel{?}{=} \mle{L}^{(k)}(r^{(k)})$ (the root layer has no node index, so we write $r_x^{(k)} \coloneqq r^{(k)}$), the parties run the binary-tree GKR grand-product argument~\cite{Thaler:2013aa} for $j = k, k - 1, \ldots, 1$. At layer $j$ the parties hold a claim $\mle{L}^{(j)}(\rho^{(j)}, r_x^{(j)})$ at a folded node-index point $\rho^{(j)} \in K^{k - j}$ and $x$-point $r_x^{(j)} \in K^{\ell_\text{mul}}$. Expanding the product relation,
\[
  \mle{L}^{(j)}(\rho^{(j)}, r_x^{(j)}) = \sum_{i \in \BB_{k - j}} \sum_{x \in \BB_{\ell_\text{mul}}} \mle{\eq}_{k - j}(\rho^{(j)}, i) \cdot \mle{\eq}_{\ell_\text{mul}}(r_x^{(j)}, x) \cdot \mle{L}^{(j - 1)}(0, i, x) \cdot \mle{L}^{(j - 1)}(1, i, x),
\]
the parties run a $\bigl( (k - j) + \ell_\text{mul} \bigr)$-round MLE-check, whose composition polynomials have individual degree $\leq 2$. It produces the two child evaluations $\mle{L}^{(j - 1)}(0, \rho', r_x^{(j - 1)})$ and $\mle{L}^{(j - 1)}(1, \rho', r_x^{(j - 1)})$, which a line reduction~\cite{Goldwasser:2015aa} collapses into the single layer-$(j - 1)$ claim $\mle{L}^{(j - 1)}(\rho^{(j - 1)}, r_x^{(j - 1)})$ with $\rho^{(j - 1)} \in K^{k - j + 1}$. After $k$ layers the parties hold a single leaf claim
\[
  s^{(0)} \stackrel{?}{=} \mle{L}(\rho, r_x), \qquad (\rho, r_x) \in K^k \times K^{\ell_\text{mul}},
\]
at the fully folded node-index $\rho \coloneqq \rho^{(0)}$ and final $x$-point $r_x \coloneqq r_x^{(0)}$. Soundness error is the standard GKR sum over layers, $O\bigl( k(k + \ell_\text{mul}) \bigr) / |K|$.
...
\[
  s^{(0)}_i \stackrel{?}{=} \mle{L}(i, r_x), \qquad i \in \BB_k,
\]
and the verifier checks that they interpolate to the folded claim,
\[
  \sum_{i \in \BB_k} \mle{\eq}_k(\rho, i) \cdot s^{(0)}_i \stackrel{?}{=} s^{(0)} = \mle{L}(\rho, r_x).
\]
...
\[
  \mle{L}(i, X) = \sel\bigl( \mle{z}(i, X),\, g^{2^i} \bigr) = 1 + (g^{2^i} - 1) \cdot \mle{z}(i, X),
\]
since $g^{2^i}$ is constant and $\mle{z}(i, X)$ is multilinear in $X$.

The parties run the product-tree GKR loop (\S\ref{sec:exp-subprotocols}) on the root claim $s^{(k)} \stackrel{?}{=} \mle{\textsf{gen-exp}}(z)(r^{(k)})$, folding to the single leaf claim $s^{(0)} \stackrel{?}{=} \mle{L}(\rho, r_x)$ at the common point $r'_x \coloneqq r_x$. Because the powers $g^{2^i}$ are public, the verifier can evaluate the leaf selector itself, so the prover re-exposes the exponent rather than the leaves: it sends the $2^k$ values
\[
  \alpha_{z, i} \stackrel{?}{=} \mle{z}(i, r'_x), \qquad i \in \BB_k,
\]
...
\[
  \sum_{i \in \BB_k} \mle{\eq}_k(\rho, i) \cdot \sel\bigl( \alpha_{z, i},\, g^{2^i} \bigr) \stackrel{?}{=} s^{(0)} = \mle{L}(\rho, r_x).
\]
...
The variable-base subprotocol reduces a claim $s^{(k)} \stackrel{?}{=} \mle{\textsf{var-exp}}(f, z)(r^{(k)})$ to one claim on $\mle{f}$ and the $2^k$ per-bit claims on $\mle{z}$. The parties first run the product-tree GKR loop and leaf extrapolation (\S\ref{sec:exp-subprotocols}), obtaining $2^k$ leaf claims $s^{(0)}_i \stackrel{?}{=} \mle{L}(i, r_x)$ at the common point $r_x$. Unlike the fixed-base case, the leaves couple the base and the exponent: the entry $L[i, x] = \sel(z[i, x], f[x]^{2^i}) = 1 + z[i, x] \cdot (f[x]^{2^i} - 1)$. Naively reducing the leaf claim to a claim on $\mle{f}$ would involve the polynomial $\mle{f}(X)^{2^i}$, whose degree blows up; we sidestep this using the Frobenius endomorphism $\varphi(\alpha) = \alpha^2$ (\S\ref{sec:binary-fields}). Applying $\varphi^{-i}$ (well-defined because $\varphi$ is invertible on $K$, with inverse $\varphi^{\lambda - 1}$) to the leaf claim:
\[
  \varphi^{-i}(s^{(0)}_i) \stackrel{?}{=} \sum_{x \in \BB_{\ell_\text{mul}}} \mle{\eq}_{\ell_\text{mul}}\bigl( \varphi^{-i}(r_x), x \bigr) \cdot \bigl( 1 + \mle{z}(i, x) \cdot (\mle{f}(x) - 1) \bigr),
\]
using that $\varphi^{-i}$ is a ring automorphism that fixes the $\FF{2}$-coefficients of $\mle{\eq}$ and of $\mle{z}(i, \cdot)$, and that $\varphi^{-i}(\mle{f}(x)^{2^i}) = \mle{f}(x)$.

The right-hand side is now of degree $\leq 2$ in $\mle{f}$ and $\mle{z}(i, \cdot)$. The verifier samples a single vector $\gamma \gets K^k$ and the parties run a batched $\ell_\text{mul}$-round sumcheck, using the multilinear Lagrange weights $\mle{\eq}_k(\gamma, i)$ as batching coefficients in place of $2^k$ independent ones:
\[
  \sum_{i = 0}^{2^k - 1} \mle{\eq}_k(\gamma, i) \cdot \varphi^{-i}(s^{(0)}_i) \stackrel{?}{=} \sum_{x \in \BB_{\ell_\text{mul}}} \sum_{i = 0}^{2^k - 1} \mle{\eq}_k(\gamma, i) \cdot \mle{\eq}_{\ell_\text{mul}}\bigl( \varphi^{-i}(r_x), x \bigr) \cdot \bigl( 1 + \mle{z}(i, x) \cdot (\mle{f}(x) - 1) \bigr).
\]
A single $\gamma$ suffices because the weights $\mle{\eq}_k(\gamma, i)$ are the values at $\gamma$ of the $k$-variate multilinear Lagrange basis, so a false claim survives with probability at most $k / |K|$ by Schwartz--Zippel. At the end, the parties hold a final sumcheck challenge $r'_x \in K^{\ell_\text{mul}}$ and claimed evaluations $\alpha_f \stackrel{?}{=} \mle{f}(r'_x)$ and $\alpha_{z, i} \stackrel{?}{=} \mle{z}(i, r'_x)$ for $i \in \set{0, \ldots, 2^k - 1}$. The verifier evaluates each $\mle{\eq}_{\ell_\text{mul}}(\varphi^{-i}(r_x), r'_x)$ on its own, costing at most $\lambda$ squarings per $i$ to compute $\varphi^{-i}(r_x)$, and closes out the batched sumcheck. The claim $\alpha_f \stackrel{?}{=} \mle{f}(r'_x)$ is the output base claim, and the $2^k$ per-bit claims $\alpha_{z, i} \stackrel{?}{=} \mle{z}(i, r'_x)$ are the output exponent claims.
...
\[
  \mle{A} \coloneqq \mle{\textsf{gen-exp}}(a), \qquad \mle{C}_\text{lo} \coloneqq \mle{\textsf{gen-exp}}(c_\text{lo}), \qquad \mle{C}_\text{hi} \coloneqq \mle{\textsf{gen-exp}}(c_\text{hi}),
\]
the multilinear extensions of the fixed-base exponentiation vectors $A, C_\text{lo}, C_\text{hi}$, whose entries $A[x] = g^{\intval{a}[x]}$, $C_\text{lo}[x] = g^{\intval{c_\text{lo}}[x]}$, and $C_\text{hi}[x] = g^{\intval{c_\text{hi}}[x]}$ all lie in $K^*$. Only the $2^{64}$-th power of $C_\text{hi}$ ever enters the relation, so write $D_\text{hi} \coloneqq C_\text{hi}^{2^{64}}$ for the high-half factor, with entries
\[
  D_\text{hi}[x] = C_\text{hi}[x]^{2^{64}} = \varphi^{64}\bigl( C_\text{hi}[x] \bigr) = \bigl( g^{2^{64}} \bigr)^{\intval{c_\text{hi}}[x]} \in K^*,
\]
the third equality recording that raising to the $2^{64}$-th power is the Frobenius $\varphi^{64}$. We write $\mle{D}_\text{hi}$ for its multilinear extension. While $D_\text{hi}$ is itself a fixed-base exponentiation with base $g^{2^{64}}$, the entrywise relation $D_\text{hi} = \varphi^{64}(C_\text{hi})$ will let phase~2 trade it for a base-$g$ claim on $C_\text{hi}$ at a twisted point. In this notation identity \eqref{eq:exp-identity} becomes the pointwise relation
\[
  \textsf{var-exp}(A, b)[x] = C_\text{lo}[x] \cdot D_\text{hi}[x], \qquad x \in \BB_{\ell_\text{mul}},
\]
since $\textsf{var-exp}(A, b)[x] = A[x]^{\intval{b}[x]} = g^{\intval{a}[x] \cdot \intval{b}[x]}$ while $C_\text{lo}[x] \cdot D_\text{hi}[x] = g^{\intval{c_\text{lo}}[x] + 2^{64} \cdot \intval{c_\text{hi}}[x]}$. The base $A$ of the variable-base exponentiation is itself the output of a fixed-base exponentiation, so verifying the relation chains the two subprotocols of \S\ref{sec:exp-subprotocols}.
...
\[
  y_0 \stackrel{?}{=} \mle{\textsf{var-exp}}(A, b)(r_0), \qquad\qquad y_0 \stackrel{?}{=} \sum_{x \in \BB_{\ell_\text{mul}}} \mle{\eq}_{\ell_\text{mul}}(r_0, x) \cdot C_\text{lo}[x] \cdot D_\text{hi}[x],
\]
...
\[
  s^{(0)}_i \stackrel{?}{=} \mle{L}(i, r_1), \qquad L[i, x] = 1 + \bigl( A[x]^{2^i} - 1 \bigr) \cdot b[i, x], \qquad i \in \BB_6,
\]
...
\[
  \sum_{i \in \BB_6} \mle{\eq}_6(\gamma, i) \cdot \varphi^{-i}\bigl( s^{(0)}_i \bigr) \stackrel{?}{=} \sum_{x \in \BB_{\ell_\text{mul}}} \sum_{i \in \BB_6} \mle{\eq}_6(\gamma, i) \cdot \mle{\eq}_{\ell_\text{mul}}\bigl( \varphi^{-i}(r_1), x \bigr) \cdot \bigl( 1 + (\mle{A}(x) - 1) \cdot \mle{b}(i, x) \bigr),
\]
a degree-$2$ sumcheck in $\mle{A}$ and $\mle{b}$.

\paragraph{Phase 2 (batched sumcheck and high-half twist).}
...
\[
  y_0 \stackrel{?}{=} \sum_{x \in \BB_{\ell_\text{mul}}} \mle{\eq}_{\ell_\text{mul}}(r_0, x) \cdot \mle{C}_\text{lo}(x) \cdot \mle{D}_\text{hi}(x),
\]
a degree-$2$ sumcheck in $\mle{C}_\text{lo}$ and $\mle{D}_\text{hi}$. Sharing the common challenge $r_2 \in K^{\ell_\text{mul}}$, the sumchecks output the root claims $\mle{A}(r_2)$ and $\mle{C}_\text{lo}(r_2)$ and the high-half claim $\mle{D}_\text{hi}(r_2)$. The variable-base subprotocol outputs the $64$ per-bit exponent claims $\mle{b}(i, r_2)$ at the constraint point $r_2$; the verifier recombines them into the single claim $\mle{b}(r_I^b, r_2) = \sum_{i \in \BB_6} \mle{\eq}_6(r_I^b, i) \, \mle{b}(i, r_2)$ by sampling $r_I^b \gets K^6$, carrying one exponent claim rather than $64$ into the later phases.

Finally, the verifier rewrites the base-$g^{2^{64}}$ claim $\mle{D}_\text{hi}(r_2)$ as a base-$g$ claim on $\mle{C}_\text{hi}$. Because $D_\text{hi} = \varphi^{64}(C_\text{hi})$ entrywise and the Frobenius $\varphi$ is an $\FF{2}$-automorphism of $K$---so it fixes the $\set{0, 1}$ coefficients of the equality weights and distributes over the multilinear extension, up to a twist of the evaluation point---
\[
  \mle{D}_\text{hi}(X) = \varphi^{64}\bigl( \mle{C}_\text{hi}(\varphi^{-64}(X)) \bigr),
\]
and the verifier replaces $\mle{D}_\text{hi}(r_2)$ by the equivalent claim $\mle{C}_\text{hi}(\varphi^{-64}(r_2)) = \varphi^{-64}\bigl( \mle{D}_\text{hi}(r_2) \bigr)$, computed locally. The phase hands the three base-$g$ root claims $\mle{A}(r_2)$, $\mle{C}_\text{lo}(r_2)$, $\mle{C}_\text{hi}(\varphi^{-64}(r_2))$ to phase~3, and the single recombined exponent claim $\mle{b}(r_I^b, r_2)$ to phase~4.

\paragraph{Phase 3 (fixed-base trees).}
The parties run the three base-$g$ fixed-base product trees on $\mle{A}(r_2)$, $\mle{C}_\text{lo}(r_2)$, $\mle{C}_\text{hi}(\varphi^{-64}(r_2))$ (the $\mle{C}_\text{hi}$ tree rooted at the twisted point; the differing root points enter only as distinct equality weights). The binary-tree grand-product loop (\S\ref{sec:exp-subprotocols}) is run down through \emph{all but the last layer}, folding both the bit-index and the constraint variable and batching the three trees to a common point $r_3$. Phase~3 ends holding, for each of $a, c_\text{lo}, c_\text{hi}$, an MLE-check at $r_3$ on the product of the two layer-$0$ child \emph{selectors}: writing $\mle{L}_{c_\text{lo}}$ for the gen-exp leaf with boolean values $\mle{L}_{c_\text{lo}}[i, x] = \sel(c_\text{lo}[i, x], g^{2^i})$, the claim is on $\mle{L}_{c_\text{lo}}(0, \rho, r_3) \cdot \mle{L}_{c_\text{lo}}(1, \rho, r_3)$ with $\rho \in K^5$ the folded upper bit-index, and analogously for $a$ and $c_\text{hi}$. No local inversion is performed: with the bit-index folded, the selectors are kept symbolic and the conversion to $\mle{c}_\text{lo}$ is deferred to phase~5.
...
\item the re-randomization of the exponent: an MLE-check point-switching the single claim $\mle{b}(r_I^b, r_2)$ from the phase-$2$ constraint point $r_2$ to the shared point $r_x$;
\item the \emph{wraparound parity} zerocheck \eqref{eq:lsb-parity}, $\sum_{x \in \BB_{\ell_\text{mul}}} \mle{\eq}_{\ell_\text{mul}}(r_2, x) \bigl[ \mle{a}(0, x) \, \mle{b}(0, x) - \mle{c}_\text{lo}(0, x) \bigr] \stackrel{?}{=} 0$, reusing the phase-$2$ challenge $r_2$---available for free because the $\mle{b}$ re-randomization already evaluates at constraint $r_2$.
\end{itemize}
The batch folds the constraint variable to the shared point $r_x \in K^{\ell_\text{mul}}$ and the tree bit-index to $r_I \in K^6$. Its outputs are the three leaf-selector claims $\mle{L}_a(r_I, r_x)$, $\mle{L}_{c_\text{lo}}(r_I, r_x)$, $\mle{L}_{c_\text{hi}}(r_I, r_x)$, the exponent claim $\mle{b}(r_I^b, r_x)$, and the bit-$0$ wraparound claims $\mle{a}(0, r_x)$, $\mle{b}(0, r_x)$, $\mle{c}_\text{lo}(0, r_x)$.

\paragraph{Phase 5 (leaf reconstruction).}
The prover sends the $4 \times 64$ per-bit values $\mle{a}(i, r_x), \mle{b}(i, r_x), \mle{c}_\text{lo}(i, r_x), \mle{c}_\text{hi}(i, r_x)$ for $i \in \BB_6$; these are the IntMul output. The verifier binds them to the phase-$4$ claims rather than collapsing them: for each of $a, c_\text{lo}, c_\text{hi}$ it forms the selector recombination
\[
  \sum_{i \in \BB_6} \mle{\eq}_6(r_I, i) \, \sel\bigl( \mle{a}(i, r_x), g^{2^i} \bigr) \stackrel{?}{=} \mle{L}_a(r_I, r_x),
\]
for $b$ it tests $\sum_{i \in \BB_6} \mle{\eq}_6(r_I^b, i) \, \mle{b}(i, r_x)$ against $\mle{b}(r_I^b, r_x)$, and it tests the $i = 0$ values against the wraparound claims. The sent values are thereby bound to the GKR-verified products, and to the committed witness by the shift reduction downstream.
...
For each of the four exponentiations, the prover initializes the leaf tables $\bigl( L[i, x] \bigr)_{x \in \BB_{\ell_\text{mul}}}$ for $i = 0, \ldots, 63$ from the base values $f[x]$ and the bits $z[i, x]$. Since each $z[i, x] \in \FF{2}$, the entry $L[i, x]$ is either $1$ or $f[x]^{2^i} = \varphi^i(f[x])$. The three fixed-base exponentiations all use base $f[x] \equiv g$, so the powers $g^{2^i}$ can be precomputed once into a single table indexed by $i$, and the leaves are obtained by a bit-controlled select. The high-half values $D_\text{hi}[x] = \varphi^{64}(C_\text{hi}[x])$ needed for the deferred right-hand-side sumcheck are obtained from the $C_\text{hi}$ table by a single Frobenius application, not by a separate exponentiation. For the variable-base exponentiation (base $\mle{A}$, exponent $b$), the prover applies $\varphi^i$ to each entry of the $\mle{A}$-table, which costs $i$ squarings per entry.

\paragraph{Sumcheck round computation.}
Each layer of the GKR loop runs a batched sumcheck with composing polynomials of individual degree $2$ (plus the equality indicator), over $\ell_\text{mul}$ rounds. Standard sumcheck-prover algorithms (e.g.\ Thaler~\cite[\S 4.1]{Thaler:2022aa}) apply directly. The Frobenius step's sumcheck has composing polynomials of degree $\leq 2$ in $\mle{f}$ and $\mle{z}(i, \cdot)$ (plus the equality indicator), so the per-round prover work is also linear in the table size.

\paragraph{Frobenius application.}
Computing $\varphi^{-i}(r_x)$ for $i = 0, \ldots, 63$, needed by the verifier in the variable-base Frobenius step, costs at most $\lambda \cdot 64$ field squarings in total. The high-half twist of phase~2 needs the one additional point $\varphi^{-64}(r_2)$ (a further $\lambda - 64$ squarings, or simply $\varphi^{64}(r_2)$ when $\lambda = 128$). The prover precomputes these quantities once and reuses them across the batched sumcheck.
