...
\item \textbf{IntMul reduction (\S\ref{sec:intmul}).} A separate GKR-based reduction transforms the satisfaction of the $n_\text{mul}$ IntMul constraints into evaluation claims on the four multilinear extensions $\mle{a}, \mle{b}, \mle{c}_\text{lo}, \mle{c}_\text{hi}$ of the IntMul constraint arrays.
\item \textbf{BinMul reduction (\S\ref{sec:binmul}).} A separate zerocheck-based reduction transforms the satisfaction of the $n_\text{binmul}$ BinMul constraints into per-bit evaluation claims on the multilinear extensions of the six BinMul constraint arrays $a_\text{lo}, a_\text{hi}, b_\text{lo}, b_\text{hi}, c_\text{lo}, c_\text{hi}$.
...
Phases 2--4 may be executed in any order; their outputs are independent until they are batched in Phase 5.
...
For exposition we describe the reduction for a single oblong-multilinearized constraint array $\widehat{z}(\widehat{I}, X)$ over $D \times \BB_{\ell_\text{cons}}$, with a single input evaluation claim $\alpha_z \stackrel{?}{=} \widehat{z}(r_{\widehat{i}}, r'_x)$. The protocol extends to multiple constraint arrays at the same point by random batching, with the verifier sampling fresh batching coefficients before phase 1. In the full Binius64 LIOP, the claims from the BitAnd and IntMul reductions are coordinated to share a common $(r_{\widehat{i}}, r'_x)$ via a preliminary batching step (with constraint arrays padded to a common $\ell_\text{cons} \coloneqq \max(\ell_\text{and}, \ell_\text{mul})$). The IntMul reduction contributes, for each of its four arrays, the $64$ per-bit claims $\mle{z}(r_x, i)$ over the bit-index $i \in \BB_6$ (written in the constraint-first indexing of \S\ref{sec:exp-subprotocols}); this step folds them along the bit-index---sampling $r_{\widehat{i}} \in K^6$ and recombining---at the same time as it batches across arrays, yielding the common oblong point $r_{\widehat{i}}$. A single shift reduction then processes the batched claim.
