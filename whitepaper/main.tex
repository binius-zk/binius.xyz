...
\]
The prover holds the two $I$-dependent factors as separate $2^6$-entry tables, $d[i] = \delta_D(r_{\widehat{i}}, \widehat{i})$ from the precomputation above and $v[i] = \mle{\textsf{shift-ind}}(i, r_j, r_s, r_o)$ from $2^6$ evaluations of the succinct arithmetizations of \S\ref{sec:shift-indicators}; keeping the two apart is what makes the round polynomials degree $2$.

The scalar $G$ carries the summation over $y$ of $\mle{Z}_\text{cons} \cdot \mle{w}$, and the prover obtains it by division rather than by summing: it forms $h(r_j, r_s, r_o) = \sum_{i \in \BB_6} d[i] \cdot v[i]$, a $2^6$-term sum, and sets $G \coloneqq \beta / h(r_j, r_s, r_o)$. The phase therefore costs $O(2^6)$ field operations and one inversion, and touches no table indexed by $y$. In the exceptional case $h(r_j, r_s, r_o) = 0$ the prover instead computes $G$ directly, as the inner product of the two phase-4 tables below. Phase 3 closes with the claim
\[
