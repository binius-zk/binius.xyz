...
There's a useful array-based reformulation of this condition. We can define arrays $a$, $b$ and $c$, containing $n_\text{and}$ 64-bit words, by setting, for each $x \in \{0, \ldots , n_\text{and} - 1\}$:
...
Having defined these arrays, we can say that the constraint system is fulfilled if and only if
