...
In all of our applications in this site, we will use $k = 6$. We thus let $k = 6$ once and for all, so that the $D \subset \mathbb{F}_{2^{128}}$ is a 6-dimensional $\mathbb{F}_2$-linear subspace. We will as above identify $\mathcal{B}_6 \cong D$ by basis combination, by sending $i \mapsto \widehat{i} \coloneqq i_0 \cdot \zeta_0 + \cdots + i_5 \cdot \zeta_0$.
