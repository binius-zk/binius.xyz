...
In this page, we explain how to exponentiate a variable base by a variable exponent pointwise on the cube. That is, we explain how to algebraically relate the multilinear extension of this exponentiation to the multilinear extensions of the base and the exponent. This protocol is the technical core of our multiplication protocol.

We fix an $\ell_\text{mul}$-variate multilinear $\widetilde{V}(X)$ with values in $\mathbb{F}_{2^{128}}$, and an $\mathbb{F}_2$-valued, [oblong-multilinear](/blueprint/math/oblong) exponent function $\widehat{z}(\widehat{I}, X_0, \ldots , X_{\ell_\text{mul} - 1})$. As a notational device, we write $z_i(x) \coloneqq \widehat{z}(\widehat{i}, x)$ for each $i \in \{0, \ldots , 63\}$ and $x \in \mathcal{B}_{\ell_\text{mul}}$. For each $x \in \mathcal{B}_{\ell_\text{mul}}$, we define the _exponent_ $Z(x)$ by combining $\widehat{z}(\widehat{I}, x)$'s bits on $D$:
$$
...
- **for** each tree layer $k \in \{0, \ldots , 5\}$ **do**:
  ...
  - **for** each $i \in \{0, \ldots, 2^{k + 1} - 1\}$ **do**: the prover sends the _claimez_ evaluation $s^{(k + 1)}_i = \widetilde{W^{(k + 1)}_i}(r^{(i + 1)})$.
...
Indeed, for each $i \in \{0, \ldots , 63\}$, applying $\varphi^{-i}$ to both sides of the above equality, we the equivalent claim:
...
We thus now have 64 evaluation evaluation claims about _low-degree_ multivariates. We can now apply the usual batch sumcheck to these. The verifier samples and sends to the prover 64 random scalars $\gamma_i$, for $i \in \{0, \ldots , 63\}$; the parties then sumcheck the claim:

$$
\begin{align*}
\sum_{i = 0}^{63} \gamma_i \cdot \varphi^{-1}\left( s^{(6)}_i \right) &\stackrel{?}= \sum_{i = 0}^{63} \gamma_i \cdot \left[ \sum_{x \in \mathcal{B}_{\ell_\text{mul}}} \widetilde{\texttt{eq}}\left(\varphi^{-i} \left( r_x \right), x \right) \cdot \left( 1 + z_i(x) \cdot \left( \widetilde{V}(x) - 1 \right) \right) \right] \\
...
To see why this is correct, we note that the equality of polynomials
$$
\begin{equation*}\widehat{z}(\widehat{I}, X) = \sum_{i = 0}^{63} \delta_D(\widehat{I}, \widehat{i}) \cdot \widehat{z}(\widehat{i}, X)\end{equation*}
$$
holds, since $\widehat{z}(\widehat{I}, X)$ is oblong-multilinear. Specializing both sides to $r_{\widehat{i}}$ and $r'_x$, we get the correctness of this step. As for soundness, if $\alpha_{z, i^*} \neq \widehat{z}(\widehat{i}^*, r'_x)$ holds for some $i^* \in \{0, \ldots , 63\}$, then the degree-63 univariate polynomials
