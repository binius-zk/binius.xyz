...
In this page, we describe a highly optimized implementation, which strategically uses the [Rijndael field](/blueprint/backend/ands/rijndael#the-rijndael-field) "where possible". We call our algorithm the _Rijndael zerocheck_.
...
We proceed with the prover's algorithm. The prover's inputs are the constraint arrays $a$, $b$ and $c$ and the verifier's partially-deterministic challenge $\overline{r_x} = (\sigma_0, \sigma_1, \sigma_2, \ldots , \overline{r_{x, 0}}, \ldots , \overline{r_{x, \ell_\text{and} - 4}})$, as well as the fixed Rijndael field [elements](/blueprint/backend/ands/rijndael#tensor-basis) $\rho_0$, $\rho_1$, $\rho_2$, and the precomputed mapping $\text{Extrap}$ above.
