...
Thus far, we have let $D \subset \mathbb{F}_{2^{128}}$ be an arbitrary $6$-dimensional subspace. Now, we enforce that $D \subset \mathbb{F}_{2^8} \subset \mathbb{F}_{2^{128}}$. We write $D' \subset \mathbb{F}_{2^{128}}$ for a further linear subspace, now of dimension 7 over $\mathbb{F}_2$. We require that $D' \subset \mathbb{F}_{2^8}$ too, so we have a chain of inclusions
